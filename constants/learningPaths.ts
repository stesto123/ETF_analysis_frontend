  ...
  beginner: [
    ...
    },
    {
      ...
      lessons: [
        ...
        {
          id: 'lesson-portfolio-compose',
          title: 'How to compose the right Portfolio for You',
          duration: '10 min',
          content: [
            'Goal: build the right mix for your horizon, risk tolerance, and goals (not “the best ETF in the world”).',
            'Horizon buckets: 0–3y short-term (protect capital); 3–10y mixed; 10y+ equity-led. If you need the money soon, prioritize stability over growth.',
            'Risk & volatility: more equity = bigger swings; more bonds/cash = calmer but lower long-term return. If -30% makes you sell, your equity slice is too high.',
            'Asset types: equities = growth and volatility; bonds = stability and coupons; commodities/gold = hedges/psychology, optional for beginners.',
            'Sectors: tech (high growth, volatile), staples (steady), discretionary (cycle-sensitive), healthcare (resilient), financials (rate-sensitive), energy (cyclical). Broad market ETFs already diversify across sectors.',
            'Geography: global “all-world” as base; US = innovation-heavy; Europe = traditional mix; emerging = higher growth + higher risk.',
            'Rules of thumb by horizon: 0–3y mostly bonds/cash; 3–10y blend of equity + bonds; 10y+ equity as engine with optional bonds/gold for smoother ride.',
            'Rules by risk tolerance: risk-averse = higher bonds + a bit of global equity; balanced = global equity + bond mix; risk-tolerant = high global equity with optional emerging/sector tilts.',
            'Simplicity: you don’t need 15 ETFs. A global equity ETF plus a bond ETF can be a solid core; tilts (sectors, commodities, regions) are optional layers.',
            'Big idea: start simple, stay invested, adjust later as your horizon or stomach changes.',
          ],
          takeaways: [
            'Match allocation to time horizon and your real drawdown tolerance.',
            'Use equities for long-term growth, bonds for stability, gold/commodities sparingly as diversifiers.',
            'Simplicity wins: a global equity ETF plus a bond ETF can be a strong core; tilts are optional layers.',
          ],
          ctas: [
            {
              id: 'cta-portfolio-analytics',
              label: 'Try a sample mix',
              route: '/(tabs)',
              description: 'Experiment with a global equity + bond allocation in analytics.',
            },
            {
              id: 'cta-portfolio-chat',
              label: 'Ask the AI',
              route: '/(tabs)/chat',
              prompt: 'Suggest a simple ETF split for a 7-year goal and medium risk tolerance.',
            },
          ],
          aiPrompts: [
            'Draft a 3-line rule of thumb to pick equity vs bond percentage by horizon.',
            'List 3 pros and cons of adding gold to a beginner portfolio.',
            'Show two simple ETF mixes: one conservative, one aggressive.',
          ],
        },
        {
          id: 'lesson-first-etf',
          ...
          duration: '10 cards',
          content: [
            'The journey: pick a broker, pick an ETF, place your first order.',
            'Choosing a broker: low/zero fees, simple app, broad ETF selection (index/UCITS), big providers, optional fractional shares, regulated and protected.',
            'Choosing an ETF: what it tracks (e.g., S&P 500, world, emerging); beginners often start with a broad diversified index.',
            'TER: yearly fee taken inside the fund. Lower is usually better (0.07% = $0.70 per $1,000 per year).',
            'Accumulating vs distributing: ACC reinvests dividends (growth); DIST pays out cash (income). Beginners often prefer ACC for compounding.',
            'UCITS (for Europe): a regulatory standard for transparency and protection; look for UCITS compliant if you are in that region.',
            'Order types: market = buy now; limit = buy at price X; GTC keeps a limit alive; stop-loss mostly not needed for long-term ETFs.',
            'Step-by-step buy: open broker, deposit, search ETF (name/ticker), press buy, choose order type (market is fine), confirm.',
            'Timing fear: no one times perfectly; regular investing smooths bumps (monthly, per paycheck).',
            'Common fears: a global ETF doesn’t go to zero unless the world ends; small order mistakes are minor; assets stay yours even if the app crashes.',
            'After you buy: avoid staring daily, set a schedule, keep learning, stay consistent.',
          ],
          takeaways: [
            'A good broker is low-fee, simple, regulated, and offers broad ETFs (ideally UCITS if relevant).',
            'Focus on what the ETF tracks, TER, ACC vs DIST, and UCITS when applicable.',
            'Use simple orders (market/limit), invest regularly, and don’t obsess over timing.',
          ],
          ctas: [
            {
              id: 'cta-first-etf-analytics',
              label: 'Preview an ETF',
              route: '/(tabs)',
              description: 'Open analytics to inspect a broad ETF before buying.',
            },
            {
              id: 'cta-first-etf-chat',
              label: 'Ask the AI about your first order',
              route: '/(tabs)/chat',
              prompt: 'Walk me through placing my first ETF order with a market order.',
            },
          ],
          aiPrompts: [
            'Explain ACC vs DIST to a beginner and when to pick each.',
            'List 3 beginner-friendly order types and when to use them.',
            'Draft a 5-step checklist to buy a UCITS world ETF for the first time.',
          ],
        },
        {
